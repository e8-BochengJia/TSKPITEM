...
<%
    ...
    int Notice_ID = 0;
    int cate_id = tools.CheckInt(Request["noticecate_id"]);
    Notice_ID = tools.CheckInt(Request["id"]);

   
    NoticeInfo noticeinfo = notice.GetNoticeByID(Notice_ID);
    if (noticeinfo == null)
    {
        Response.Redirect("/");
    }
    ...
    {
        if (noticeinfo.Notice_IsAudit != 1 || noticeinfo.Notice_Cate != cate_id||DateTime.Today>noticeinfo.Notice_ShowTime)
        {
            Response.Redirect("/");
        }
...
<body style="background: #f5f5f9;">
    ...
    <p class="crumb-nav">
        <img src="/images/icon-nav.png">当前位置 > <a href="/">首页 </a><%=notice.GetNotice_Cate_Nav(cate_id, " &gt; ")%>
    </p>
    <div class="w-1200 clearfix" style="margin-bottom: 30px;">
        ...
        <div class="workers-right">
            ...
            <div class="news-details-tit" style="width: auto;">
                <h2><%=noticeinfo.Notice_Title %></h2>
                <div class="news-details-date clearfix">
                    <span>添加时间：<%=noticeinfo.Notice_Addtime.ToString("yyyy-MM-dd") %>  </span>
