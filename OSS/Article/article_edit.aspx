...
<script runat="server">
    ...
    private string Article_Title, Article_Content, Article_Img, Article_Site, Article_Intro, Article_Keyword, Article_Source, Article_Author, Article_Hyperlink, Artide_ShoulderTitle, Article_CateIDs;
    private int Article_ID,SubjectID, Article_Cate, Article_IsRecommend, Article_IsAudit,  Article_Sort,Article_ContentID,Article_PageViews,Artide_ShoulderTitleSize,Article_HyperlinkSize,Artide_IsTop;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (entity == null) {
            Public.Msg("error", "错误信息", "记录不存在", false, "{close}");
            Response.End();
        }
        else {
            ...
            Article_CateIDs = myApp.GetArticleCategoryByArticleID(Article_ID);
        }
    }
</script>
...
<script type="text/javascript">
    function getID() {
        var obj = document.getElementById("Select1").value
        if (obj == "1") {
            document.getElementById('Article00').style.display = 'none';
            document.getElementById('Article02').style.display = 'inline';

        }
        else if (obj == "0") {
            document.getElementById('Article02').style.display = 'none';
            document.getElementById('Article00').style.display = 'inline';
        }
...
<script type="text/javascript">
    ...
    function Ctrl_One(obj) {
        var problemContent = obj;
        var len = problemContent.indexOf("</p>");
        if (len > 0) {
            problemContent = problemContent.substring(0, len);
        }

        problemContent = problemContent.replace(/<[^>]+>/g, "");
        var reg = new RegExp("&ldquo;", "g");

        problemContent = problemContent.replace(reg, "\"");
        reg = new RegExp("&rdquo;", "g");
        problemContent = problemContent.replace(reg, "\"");
        reg = new RegExp("&hellip;", "g");
        problemContent = problemContent.replace(reg, "...");

        reg = new RegExp("&quot;", "g");
        problemContent = problemContent.replace(reg, "\"");

        reg = new RegExp("&bull;", "g");
        problemContent = problemContent.replace(reg, "•");

        reg = new RegExp("&nbsp;", "g");
        problemContent = problemContent.replace(reg, "");

        $("#Article_Intro").val(problemContent);
    }
</script>
                <script>
        function AuditProject(obj) {
            $("#action").val(obj);
            $("#formadd").submit();
        }
...
<div class="content_div">
    ...
    <tr>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="cell_table" id="frm_optitem_1">
           ...
           <span id="main_cate"><%=myAppC.Article_Category_Select(Article_Cate, "main_cate")%></span></td>
            ...
            <tr>
                                        ...
                                        <script type="text/javascript">
                                            ...
                                            tree.loadXML("treedata.aspx?Article_CateIDs=<%=Article_CateIDs %>");
                                        </script>
             ...
             <tr>
                                        ...
                                        <td class="cell_content">
                                            <%=mySubject.GetArticleSubjectSelect(SubjectID)%>
                                        </td>
                                    </tr>
        <tr>
          <td class="cell_title">文章来源</td>
          <td class="cell_content"><input name="Article_Source" type="text" value="<%=Article_Source %>" id="Article_Source" size="50" maxlength="50" />
        ...
        <tr>
          ...
          <td class="cell_content"><textarea name="Article_Intro" id="Article_Intro" cols="50" rows="5"><%=Article_Intro%></textarea></td>
        </tr>
        <tr>
          <td class="cell_title">预览图片</td>
          <td class="cell_content"><iframe id="iframe2" src="<% =Application["upload_server_url"]%>/public/FileUpload.aspx?App=article&formname=formadd&frmelement=Article_Img&rtvalue=1&rturl=<% =Application["upload_server_return_admin"]%>" width="100%" height="35" frameborder="0" scrolling="no"></iframe>                                    <span class="tip">建议上传190*120尺寸;高端论苑285*180;图片新闻250*160;约稿、合作220*220;声音66*75</span></td>
        </tr>
        <tr id="tr_Article_Img" <%if(Article_Img.Length==0){Response.Write("style='display:none'");} %>>
         ...
         <tr>
          ...
          <td class="cell_content"><iframe id="iframe1" src="<% =Application["upload_server_url"]%>/public/FileUpload.aspx?App=content&formname=formadd&frmelement=Article_Content&rtvalue=1&rturl=<% =Application["upload_server_return_admin"]%>" width="100%" height="35" frameborder="0" scrolling="no"></iframe></td>
        </tr>
          <tr>
                                <td class="cell_title">
                                    上传附件
        ...
        <tr>
                                ...
                                <td class="cell_content">
                                    <input name="Article_SEO_Title" type="text" id="Article_SEO_Title" size="50" maxlength="200" value="<%=Article_SEO_Title %>"/>
                                </td>
                            </tr>
                            <tr>
                                <td class="cell_title">
                                ...
                                <td class="cell_content">
                                    <input name="Article_SEO_Keyword" type="text" id="Article_SEO_Keyword" size="50" maxlength="200" value="<%=Article_SEO_Keyword %>"/>
                                </td>
                            </tr>
                            <tr>
                                <td class="cell_title">
                                ...
                                <td class="cell_content">
                                    <textarea name="Article_SEO_Description" cols="50" rows="5" id="Article_SEO_Description"><%=Article_SEO_Description%></textarea>
                                </td>
                            </tr>

        <tr>
          <td class="cell_title">排序</td>
          <td class="cell_content"><input name="Article_Sort" type="text" id="Article_Sort" value="<%=Article_Sort %>" size="10" maxlength="10" />
            <span class="tip">数字越小越靠前</span></td>
        </tr>

        <tr>
          <td class="cell_title">是否推荐阅读</td>
  ...
  <tr>
                                ...
                                <td class="cell_content">
                                    <input name="Artide_IsTop" type="radio" id="Artide_IsTop" value="1" <% =Public.CheckedRadio(Artide_IsTop.ToString(), "1")%> />是
                                    <input type="radio" name="Artide_IsTop" id="Artide_IsTop1" value="0" <% =Public.CheckedRadio(Artide_IsTop.ToString(), "0")%> />否
                                </td>
                            </tr>
...
<div class="float_option_div" id="float_option_div">
     <input type="hidden" name="Article_CateIDs" id="Article_CateIDs" value="" />
        ...
        <input name="button" type="button" class="bt_grey" id="button" value="取消" onmouseover="this.className='bt_orange';" onmouseout="this.className='bt_grey';" onclick="location='Article_list.aspx';"/>
</div>
