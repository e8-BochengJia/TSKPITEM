...
<%  Public.CheckLogin("6b6a4750-6130-4a7d-9076-c7c97f4f5398");
    ...
    if (keyword != "输入用户、目标编号、目标名称、操作说明进行搜索" && keyword != null)
    {
        defaultkey=keyword;
    }
    else
    ...
    if (keyword == "")
    {
        defaultkey = "输入用户、目标编号、目标名称、操作说明进行搜索";
    }
    else
    {
        defaultkey = keyword;
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0" class="content_table">
    ...
    <tr>
      ...
      <form id="frmsearch" method="get" action="?">
        <table>
            <tr>
                ...
                <span class="left_nav"></span>操作时间：
                                ...
                                <input  type="hidden" name="channel" id="channel" value="<%=log_channel %>"/>
                                <input type="text" name="keyword" id="keyword" size="70" onfocus="if(this.value=='输入用户、目标编号、目标名称、操作说明进行搜索'){this.value='';}"
                        value="<% =defaultkey %>">
                </td>
        ...
        <script type="text/javascript">
        jQuery("#list").jqGrid({
            url: 'user_do.aspx?action=loglist&channel=<%=log_channel.ToString()+VisitURL %>',
			datatype: "json",
			colNames: ['时间', "用户", "目标编号", "目标名称", "操作说明", "IP"],
            colModel: [
